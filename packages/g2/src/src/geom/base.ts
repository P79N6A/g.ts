import * as Attr from "@antv/attr/lib";
import * as Adjust from "@antv/adjust/lib";
import * as Base from "../base";
import * as Util from "../util";
import * as Global from "../global";
import * as Labels from "./label/index";
import * as Shape from "./shape/shape";
import * as TooltipMixin from "./mixin/tooltip";
import * as ActiveMixin from "./mixin/active";
import * as SelectMixin from "./mixin/select";
import * as parseFields from "./util/parse-fields";
...
export class GeomBase extends Base {
  ...
  paint() {
    ...
    if (self.get("labelCfg")) {
      self._addLabels(
        Util.union.apply(null, mappedArray),
        shapeContainer.get("children")
      );
    }
  ...
  _sort(mappedArray) {
    ...
    Util.each(mappedArray, itemArr => {
      itemArr.sort((obj1, obj2) => {
        return (
          xScale.translate(obj1[FIELD_ORIGIN][xField]) -
          xScale.translate(obj2[FIELD_ORIGIN][xField])
        );
      });
    });
    self.set("dataArray", mappedArray);
  ...
  _addLabels(points, shapes) {
    ...
    const container = self.get("container");
    const scales = Util.map(self.get("labelCfg").fields, field =>
      self._createScale(field)
    );
        ...
        if (attrs.hasOwnProperty(k)) {
          ...
          if (names.length > 1) {
            ...
            for (let j = 0; j < values.length; j++) {
              ...
              const name = names[j];
              newRecord[name] =
                Util.isArray(val) && val.length === 1 ? val[0] : val; // 只有一个值时返回第一个属性值
            }
          } else {
