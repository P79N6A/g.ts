...
function getAnimateParam(animateCfg, index, id) {
  const result = {};
  if (animateCfg.delay) {
    result.delay = Util.isFunction(animateCfg.delay)
      ? animateCfg.delay(index, id)
      : animateCfg.delay;
  }
  result.easing = Util.isFunction(animateCfg.easing)
    ? animateCfg.easing(index, id)
    : animateCfg.easing;
  result.duration = Util.isFunction(animateCfg.duration)
    ? animateCfg.duration(index, id)
    : animateCfg.duration;
...
function scaleInY(shape, animateCfg) {
  ...
  shape.animate(
    endState,
    animateParam.duration,
    animateParam.easing,
    animateParam.callback,
    animateParam.delay
  );
}
...
function fadeIn(shape, animateCfg) {
  const id = shape._id;
  const index = shape.get("index");
  const fillOpacity = Util.isNil(shape.attr("fillOpacity"))
    ? 1
    : shape.attr("fillOpacity");
  const strokeOpacity = Util.isNil(shape.attr("strokeOpacity"))
    ? 1
