...
export class Tooltip extends Base {
  getDefaultCfg() {
    return {
      ...
      itemTpl:
        "<li data-index={index}>" +
        '<span style="background-color:{color};" class=' +
        MARKER_CLASS +
        "></span>" +
        "{name}<span class=" +
        VALUE_CLASS +
  ...
  _init() {
    ...
    if (crosshairs) {
      if (crosshairs.type === "rect") {
        this.set(
          "crosshairs",
          Util.deepMix({}, viewTheme.tooltipCrosshairsRect, crosshairs)
        );
        ...
        this.set(
          "crosshairs",
          Util.deepMix({}, viewTheme.tooltipCrosshairsLine, crosshairs)
        );
    ...
    const attrs = Util.mix(
      {
        ...
        width: plotRange
          ? plotRange.br.x - plotRange.bl.x
          : canvas.get("width"),
        height: plotRange
          ? Math.abs(plotRange.tl.y - plotRange.bl.y)
          : canvas.get("height")
      },
      style
    );
  ...
  isContentChange(title, items) {
    ...
    let isChanged = !(
      title === titleContent && lastItems.length === items.length
    );
    if (!isChanged) {
      Util.each(items, (item, index) => {
  ...
  setPosition(x, y, target) {
    ...
    if (this.get("inPlot")) {
      // tooltip 必须限制在绘图区域内
      const plotRange = this.get("plotRange");
      position = confineTooltipPosition(
        x,
        y,
        container,
        plotRange,
    ...
    if (prePosition.x !== x || prePosition.y !== y) {
      ...
      if (crosshairsRectShape) {
        ...
        if (this.get("crosshairs").width) {
          // 用户定义了 width
          crosshairsRectShape.attr(
            dim,
            startDim - this.get("crosshairs").width / 2
          );
          ...
          if (Util.isArray(firstItem.point[dim]) && !firstItem.size) {
            ...
            if (items.length === 1) {
              ...
              crosshairsRectShape.attr(
                attr,
                Math.abs(lastItem[dim] - firstItem[dim]) + 2 * offset
              );
            }
          }
        }
