...
import * as Components from "@antv/component/lib";
import * as Global from "../../global";
const { Legend } = Components;
const { Category } = Legend;
export class Tail extends Category {
  getDefaultCfg() {
  ...
  _addItem(item) {
    ...
    this.get("appendInfo") &&
      textShape.setSilent("appendInfo", this.get("appendInfo"));
    // 添加一个包围矩形，用于事件支持
    ...
    this.get("appendInfo") &&
      wrapperShape.setSilent("appendInfo", this.get("appendInfo"));
      ...
      b.targets.forEach(function() {
        ...
        if (dist > elementHeight / 2) {
          self._adjustDenote(
            group,
            y,
            origin_y - self.get("group").attr("matrix")[7] / 2
          );
        }
