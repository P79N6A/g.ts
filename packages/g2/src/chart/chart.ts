import * as Plot from '../component/plot';
...
export class Chart extends View {
  ...
  _initCanvas() {
    ...
    const wrapperEl = DomUtil.createDom(
  ...
  _initPlotBack() {
    ...
    const plot      = canvas.addGroup(Plot, {
      ...
      plotBackground: Util.mix(
        {},
        viewTheme.plotBackground,
        self.get('plotBackground')
      ),
  ...
  _initEvents() {
    if (this.get('forceFit')) {
      window.addEventListener(
        'resize',
        Util.wrapBehavior(this, '_initForceFitEvent')
      );
    }
  }
